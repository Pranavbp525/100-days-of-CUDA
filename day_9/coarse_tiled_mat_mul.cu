...
__global__ void tiledMatrixMulKernel(float* inp1, float* inp2, float* out, int size) {
    ...
    int j_start = threadIdx.x + blockIdx.x * TILE_SIZE * COARSE_FACTOR;
    ...
    for(int phase = 0; phase < num_phases; ++phase){
        ...
        for (int c = 0; c < COARSE_FACTOR; c++){
            ...
            int inp2_col_index = j_start + c*TILE_SIZE;

            inp2_s[threadIdx.y][threadIdx.x] = inp2[inp2_row_index*size + inp2_col_index];
            __syncthreads();

            for(int k = 0; k < TILE_SIZE; ++k){
                val[c] += inp1_s[threadIdx.y][k]*inp2_s[k][threadIdx.x];
            }
            __syncthreads();
