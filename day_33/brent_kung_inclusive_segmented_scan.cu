...
__global__ void brent_kung_inclusive_scan_kernel(float * X, float * Y, unsigned int N){
...
void brent_kung_inclusive_scan_host(float *X_h, float *Y_h, unsigned int N) {
    if (N > SECTION_SIZE) {
    ...
    brent_kung_inclusive_scan_kernel<<<gridDim, blockDim>>>(X_d, Y_d, N);
...
int main() {
    ...
    brent_kung_inclusive_scan_host(X_h, Y_h, N);

    cout << "Inclusive Scan Result :" << endl;
    for (unsigned int i = 0; i < N; i++) {
        cout << Y_h[i] << " ";
    }
    cout << endl;
